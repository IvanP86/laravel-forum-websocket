        ...
        <div class="flex items-center mb-8">
            ...
            <template v-if="checkGlobalAuth()">
                <Link :href="route('sections.create')"
                    class="block mr-4 w-1/6 px-2 py-1 bg-white border border-x-gray-300 rounded-lg text-center">+ Раздел
                </Link>
            </template>
            <template v-if="sections.filter( section => checkSectionAuth(section)).length">
                <Link :href="route('branches.create')"
                    class="block w-1/6 px-2 py-1 bg-white border border-x-gray-300 rounded-lg text-center">+ Ветка
                </Link>
            </template>
        </div>
        <div v-if="sections">
